...
void TIM1_Configuration(void)//TIM1四通道分别输出PWM1/PWM2/PWM3/PWM4,对应引脚为PA8/PA9/PA10/PA11
{
    ...
    TIM_OCInitTypeDef TIM_OCInitStructure;          // 定时器比较输出初始化结构体
	...
	TIM_TimeBaseStructure.TIM_Period = PWMPeriod;                  // 定时器周期
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;                   // 时钟分频
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;               // 重复计算器
	TIM_TimeBaseInit(TIM1, &TIM_TimeBaseStructure);
	
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM2;                // 比较输出模式
	...
	TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCIdleState_Reset;    // 空闲状态下比较互补输出状态
